...
abstract class MasterFragment : Fragment()
{
    ...
    override fun onCreate(savedInstanceState: Bundle?)
    {
        ...
        loadingThread = Thread(Runnable {
            ...
            year = Year(Table.DEFAULT_LONG, context.keyStore.getPreference(Constants.SharedPrefKeys.SELECTED_YEAR_KEY, Calendar.getInstance().get(Calendar.YEAR).toLong()) as Long).apply { load(context.database) }

            val eventId = context.keyStore.getPreference(Constants.SharedPrefKeys.SELECTED_EVENT_KEY, Table.DEFAULT_LONG) as Long
            if (eventId > 0)
